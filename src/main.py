...
PROJECT = {
  ...
  "difficultyPt": "Intermediario",
  "kind": "registry",
  "dataset": "Synthetic class registry sample",
  "source": "Local synthetic sample",
  "description": "CSV registry lab for create, list, search, and remove operations using fictional records.",
  "concepts": [
    "functions",
    "dictionaries",
    "CSV files",
...
def run_cleaning() -> dict:
    ...
    for column in cleaned.columns:
        if pd.api.types.is_numeric_dtype(cleaned[column]):
            ...
            cleaned[column] = cleaned[column].fillna("not_informed")
...
def run_quality_audit() -> dict:
    ...
    html = "<h1>Data Quality Report</h1>" + report.to_html(index=False) + f"<p>Duplicate rows: {duplicate_rows}</p>"
    ensure_dirs()
...
def run_quiz() -> dict:
    ...
    return {"rows": len(result), "score_reference": int(result["points"].sum()), "outputs": [save_dataframe(result, "quiz_reference.csv")]}
...
def run_dice() -> dict:
    ...
    plt.axhline(1 / 6, color="black", linestyle="--", label="expected 1/6")
    plt.legend()
...
def run_deforestation() -> dict:
    ...
    pivot.plot(marker="o", figsize=(7, 4), title="Synthetic deforestation sample")
